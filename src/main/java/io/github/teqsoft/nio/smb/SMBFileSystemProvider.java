...
public final class SMBFileSystemProvider extends FileSystemProvider {
    ...
    public SMBFileSystem newFileSystem(URI uri, Map<String, ?> env) {
        ...
        try {
            final CIFSContext context = this.createContext(uri, env);
    ...
    public SMBFileSystem getFileSystem(URI uri) {
        ...
        /* Constructs a canonical authority string, taking all possible ways to provide credentials into consideration. */
        try {
            final CIFSContext context = this.createContext(uri, new HashMap<>());
            final String authority = this.constructAuthority(uri, context);
     ...
     */
    private CIFSContext createContext(URI uri, Map<String, ?> env) {
...
//            CIFSContext context = new BaseContext(new PropertyConfiguration(properties));
            ...
            if (Config.getBoolean(properties, "smb-nio.useNtlmPasswordAuthenticator", false)) {
              ...
              StringTokenizer userInfoTokenized = new StringTokenizer(userInfo, ":;");
              
              String domain = null;
              String username = null;
              String password = null;
              
              if (userInfoTokenized.countTokens() == 2) {
              	username = userInfoTokenized.nextToken();
              	password = userInfoTokenized.nextToken();
              } else if (userInfoTokenized.countTokens() == 3) {
              	domain = userInfoTokenized.nextToken();
              	username = userInfoTokenized.nextToken();
              	password = userInfoTokenized.nextToken();
              } else {
              	
              }
              context = context.withCredentials(new NtlmPasswordAuthenticator(domain, username, password));
            }
            return context;
